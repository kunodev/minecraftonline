...
public class HUDGUI extends GuiIngame {
	...
	public HUDGUI(Minecraft par1Minecraft)
	{
		super(par1Minecraft);
		...
		if (!this.mc.playerController.shouldDrawHUD())
		{
			...
			if(playerclient != null)
			{
				...
				if(percentHealth < 0.15){
					...
					this.drawCenteredString(fontRenderer, healthinfo, scaledWidth / 2 - 45 , healthRowHeight+1, 0xFFFFFF);
				}
				//Draw Mana
				int manaRowHeight = healthRowHeight - 10;
				int maxMana = playerclient.getMaxMana();
				...
				if(manaPercentage < 0.15){
					...
					this.drawCenteredString(fontRenderer, manaInfo, scaledWidth / 2 + 45 , manaRowHeight+1, 0xFFFFFF);
				}
				
			}
			
            FoodStats var15 = this.mc.thePlayer.getFoodStats();
            int currentfood = var15.getFoodLevel();
            int maxfood = 20;
            int foodRowHeight = scaledHeight -39;  
            float foodPercentage = (float)currentfood/maxfood;
			...
			if(currentfood < 3 ){
				this.drawCenteredString(fontRenderer, foodInfo, scaledWidth / 2 + 45 , healthRowHeight+1, 0xFF0000);
			}else{
				this.drawCenteredString(fontRenderer, foodInfo, scaledWidth / 2 + 45 , healthRowHeight+1, 0xFFFFFF);
			}
			...
			if(armorPercentage < 0.15){
				this.drawCenteredString(fontRenderer, armorInfo, scaledWidth / 2 - 45 , armorRowHeight+1, 0xFF0000);
			}else{
				this.drawCenteredString(fontRenderer, armorInfo, scaledWidth / 2 - 45 , armorRowHeight+1, 0xFFFFFF);
			}
